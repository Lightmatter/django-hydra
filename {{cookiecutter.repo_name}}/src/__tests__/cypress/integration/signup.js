describe('Singup page', () => {
  ...
  it('a user can signup with the form', () => {
    ...
    cy.get('[data-cy=password]').click().type('iwanttoknowmore');
    cy.get('[data-cy=re-password]').click().type('iwanttoknowmore');
  ...
  it('passwords must match', () => {
    ...
    cy.get('.Mui-error').should('contain', 'The Two Passwords Must Match');
  });
