...
-   `/{{ cookiecutter.project_name }}` - the Django project folder
    ...
    -   `analytics` - If analytic was used via the cookiecutter setup script, this is where analytics' views, etc. are created
...
3. set an environment variable for SENTRY_AUTH_TOKEN and TF_VAR_SENTRY_AUTH_TOKEN after getting the token through the sentry web UI
...
If the project is using analytics a new django app will be included in `analytics`. In the settings/base.py file there will be a new SEGMENT_KEY setting included. In later iterations we can/will include other 3rd party integrations.

TODO conditionally generate client-side folders and files for event tracking. This includes react and react-native for now and perhaps a generic/vanilla js solution as well.
